...
public:
  ...
  using router = coro_rpc::protocol::router<msgpack_rpc_protocol>;

  static std::optional<supported_serialize_protocols>
  get_serialize_protocol(req_header &req_header) {
    if (req_header.serialize_type == 0)
      AS_LIKELY { return msgpack_protocol{}; }
    else {
      return std::nullopt;
    }
  };
  ...
  static async_simple::coro::Lazy<std::error_code>
  read_head(Socket &socket, req_header &req_head) {
  ...
  template <typename Socket>
  static async_simple::coro::Lazy<std::error_code>
  read_payload(Socket &socket, req_header &req_head, std::string &buffer,
...
using msgpack_rpc_context =
    coro_rpc::context_base<return_msg_type,
                           coro_rpc::protocol::msgpack_rpc_protocol>;
}